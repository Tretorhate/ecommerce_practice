...
export class CartService {
  ...
  getStores(): Observable<{ id: string; title: string }[]> {
    return this.http.get<{ id: string; title: string }[]>(`/stores`);
  }
  ...
  getProductsByIds(ids: string[]): Observable<any[]> {
    const requests = ids.map((id) =>
      this.http.get<any>(`/products/by-id/${id}`),
    );
  ...
  addToCart(productId: string, storeId: string): void {
    ...
    if (
      ...
      console.log(
        `Product with ID ${productId} is already in the cart from store ${storeId}.`,
      );
    }
  }
}
